    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        body { font-family:'Plus Jakarta Sans',sans-serif; background:linear-gradient(135deg,#f0effe 0%,#ffffff 100%); min-height:100vh; display:flex; align-items:flex-start; justify-content:center; padding:40px 20px; -webkit-font-smoothing:antialiased; }
        .wrap { width:100%; max-width:620px; }
        .logo-row { display:flex; align-items:center; gap:10px; margin-bottom:32px; }
        .logo-text { font-size:1.25rem; font-weight:800; color:#1a1535; letter-spacing:-0.3px; }
        ...
        .progress-fill { height:100%; border-radius:999px; background:linear-gradient(90deg,#534AB7,#7F77DD); transition:width 0.4s ease; }
        .card { background:white; border-radius:16px; border:1px solid #e2e8f5; box-shadow:0 4px 20px rgba(26,21,53,0.08); padding:36px; margin-bottom:16px; }
        .step-label { font-size:0.7rem; text-transform:uppercase; letter-spacing:2px; color:#534AB7; font-weight:700; margin-bottom:8px; }
        .step-title { font-size:1.3rem; font-weight:800; color:#1a1535; letter-spacing:-0.3px; margin-bottom:6px; }
        ...
        .plan-card.selected { border-color:#534AB7; background:#EEEDFE; }
        .plan-card:hover:not(.selected) { border-color:#94a3b8; }
        .plan-name { font-size:0.95rem; font-weight:800; color:#1a1535; margin-bottom:4px; }
        .plan-price { font-size:1.3rem; font-weight:800; color:#534AB7; }
        ...
        .btn-primary { flex:1; padding:13px; background:linear-gradient(135deg,#534AB7,#7F77DD); color:#fff; font-size:0.9rem; font-weight:700; border:none; border-radius:10px; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; box-shadow:0 4px 14px rgba(83,74,183,0.3); transition:opacity 0.2s; }
        ...
        .success-title { font-size:1.4rem; font-weight:800; color:#1a1535; margin-bottom:8px; }
        .success-sub { font-size:0.875rem; color:#64748b; line-height:1.7; max-width:420px; margin:0 auto; }
        .required { color:#ef4444; margin-left:2px; }
        .trial-banner { background:#EEEDFE; border:1px solid #CECBF6; border-radius:10px; padding:14px 16px; margin-bottom:16px; font-size:0.84rem; color:#534AB7; line-height:1.6; width:100%; }
            ...
            <>
              ...
              {step === 2 && (
                <>
                  ...
                  <textarea className="field" value={form.services} onChange={e => update('services', e.target.value)} placeholder="List your main services, one per line" />
                  ...
                  <div className="btn-row">
                    ...
                    <button className="btn-primary" disabled={!form.agent_name || !form.business_description} onClick={() => setStep(3)}>Continue →</button>
                  </div>
                </>
              )}
              ...
              {step === 3 && (
                <>
                  ...
                  <textarea className="field" value={form.after_hours} onChange={e => update('after_hours', e.target.value)} placeholder="What should the agent tell callers outside business hours?" />
                  ...
                  <div className="btn-row">
                    ...
                    <button className="btn-primary" disabled={!form.agent_goal} onClick={() => setStep(4)}>Continue →</button>
                  </div>
                </>
              )}
              ...
              {step === 4 && (
                <>
                  ...
                  <textarea className="field" value={form.always_say} onChange={e => update('always_say', e.target.value)} placeholder="Anything the agent should always mention" />
                  <label className="field-label">Never Say or Do</label>
                  <textarea className="field" value={form.never_say} onChange={e => update('never_say', e.target.value)} placeholder="Anything the agent should avoid" />
                  <label className="field-label">Common FAQs</label>
                  <textarea className="field" style={{minHeight:'120px'}} value={form.faqs} onChange={e => update('faqs', e.target.value)} placeholder="Q: How long does an inspection take?&#10;A: Usually 2-3 hours" />
                  <div className="btn-row">
                ...
                <>
                  ...
                  {isTrial && (
                    <div className="trial-banner">
                      🎯 <strong>7-Day Free Trial</strong> — Enter your card details to start. You won't be charged until day 8. Cancel anytime before then.
                    </div>
                  )}
