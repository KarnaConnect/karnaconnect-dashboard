    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .call-card-actions { display:flex; gap:6px; margin-top:8px; }
                      ...
                      <>
                        <tr key={call.id}>
                          ...
                          <td className="actions-col">
                            {call.recording_url && (
                              <span
                                className={`action-btn ${expanded[call.id] === 'recording' ? 'active' : ''}`}
                                onClick={() => togglePanel(call.id, 'recording')}
                                title="Play Recording"
                              >🎙</span>
                            )}
                            {call.call_summary && (
                              <span
                                className={`action-btn ${expanded[call.id] === 'summary' ? 'active' : ''}`}
                                onClick={() => togglePanel(call.id, 'summary')}
                                title="View Summary"
                              >📄</span>
                            )}
                            {call.full_transcript && (
                              <span
                                className={`action-btn ${expanded[call.id] === 'transcript' ? 'active' : ''}`}
                                onClick={() => togglePanel(call.id, 'transcript')}
                  ...
                  {calls.map(call => (
                    <div key={call.id} className="call-card">
                      ...
                      <div className="call-card-actions">
                        {call.recording_url && (
                          <span
                            className={`call-card-action-btn ${expanded[call.id] === 'recording' ? 'active' : ''}`}
                            onClick={() => togglePanel(call.id, 'recording')}
                          >🎙 Recording</span>
                        )}
                        {call.call_summary && (
                          <span
                            className={`call-card-action-btn ${expanded[call.id] === 'summary' ? 'active' : ''}`}
                            onClick={() => togglePanel(call.id, 'summary')}
                          >📄 Summary</span>
                        )}
                        {call.full_transcript && (
                          <span
                            className={`call-card-action-btn ${expanded[call.id] === 'transcript' ? 'active' : ''}`}
                            onClick={() => togglePanel(call.id, 'transcript')}
                          >📋 Transcript</span>
                        )}
                      </div>
                                ...
                                {call.full_transcript.split('\n').map((line, i) => (
                                  line.trim() ? (
                                    <div key={i} className={`transcript-line ${line.startsWith('AI:') ? 'ai' : 'user'}`}>
                                      <span className="transcript-speaker">
                                        {line.startsWith('AI:') ? '⚛ Mash' : '👤 Caller'}
                                      </span>
                                      {line.replace('AI: ', '').replace('User: ', '')}
                                    </div>
                                  ) : null
